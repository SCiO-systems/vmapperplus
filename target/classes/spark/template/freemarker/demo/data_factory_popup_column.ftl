...
//        let colDef = getCurTableDef().mappings[itemData.column_index - 1];
        ...
        if (isVirtual && !itemData.column_index) {
            ...
            console.log("current col def: " + colDef.ontoterm);
        }
        if (!type) {
            if (itemData.ols_info) {
                type = "ols";
            } else if (itemData.icasa) {
            ...
            console.log("type of columnn: " + type);
        }
        let buttons = {
            cancel: {
                label: "Cancel",
...
//                    let othOpts = $(this).find("[name=other_options]").val();
                    console.log('ok - variable type: ' + curVarType);
                    console.log('ok - col def: ' + colDef.column_index);
                    console.log('ok - subdiv: ' + subDiv.val());
                    ...
                    if (!itemData.err_msg) {
                        console.log("updating data for " + colDef + " with " + curVarType);
                        ...
                        if (colDef.unit === "code" && !itemData.code_mappings_undefined_flg) {
                            if (itemData.code_mappings) {
                                ...
                                console.log("mappings: " + colDef.code_mappings);
                            ...
                            if (itemData.code_descriptions) {
                                ...
                                console.log("code descriptions: " + colDef.code_descriptions);
...
//                            columns[itemData.column_index - 1].format = "YYYY-MM-DD";
                        ...
                        console.log("updating column mapping annotation for column " + (itemData.column_index - 1));
                        console.log(getColStatusClass(itemData.column_index - 1));
                        ...
                        console.log("[name='" + curFileName + "_" + curSheetName + "_" + (itemData.column_index - 1) + "_label']");
                        ...
                        if (!itemData.code_mappings_undefined_flg) {
                            if ($('#tableViewSwitch2').prop("checked")) {
                                ...
                                console.log("table def: " + tableDef);
                        ...
                        console.log("updated data with " + curVarType);
...
//                }
        ...
        dialog.on("shown.bs.modal", function() {
            ...
            dialog.find("[name='icasa_info']").each(function () {
                ...
                //this is the snippet to adapt for ols
            ...
            // ols fields
            dialog.find("[name='ols_info']").each(function () {
                let subDiv = $(this);
                subDiv.on("type_shown", function() {
                    chosen_init_target(subDiv.find("[name='ontoterm']"), "chosen-select-deselect");
                    $(this).find(".col-def-input-item").each(function () {
                        if ($(this).attr("type") === "checkbox") {
                            if (itemData[$(this).attr("name")]) {
                                $(this).prop("checked", itemData[$(this).attr("name")]).trigger("change");
                    ...
                    $(this).find("[name='ontoterm']").trigger("change");
                });
                subDiv.find("[name='ontoterm']").each(function () {
                    $(this).on("chosen:showing_dropdown", function() {
                        if (!$(this).val()) {
                            subDiv.find(".chosen-search-input").val(itemData.column_header).trigger("paste");
                        }
                    ...
                    $(this).on("change", function () {
                        ...
                        let label = terms.getLabel(selectedTerm);
                        console.log("selected term has label: " + label);
                        if (!itemData.label || itemData.ontoterm !== selectedTerm) {
                            subDiv.find("[name='label']").val(label);
                        }
                        let iri = terms.getIri(selectedTerm);
                        console.log("selected term has iri: " + iri);
                        if (!itemData.iri || itemData.ontoterm !== selectedTerm) {
                            subDiv.find("[name='iri']").val(iri);
                        }
...
//                    }
    ...
    function updateData(div, itemData, curVarType) {
        ...
        console.log("update data - var type: " + curVarType);
        console.log("update data - subdiv: " + subDiv.val());
        let othOpts = div.find("[name=other_options]").val();
        console.log("other options: " + othOpts);
        subDiv.find(".col-def-input-item").each(function () {
            if ($(this).attr("type") === "checkbox") {
                ...
                console.log("update data - item data: " + itemData[$(this).attr("name")]);
            } else {
                delete itemData[$(this).attr("name")];
            }
        });
    ...
    function buildOlsLookup() {
        let varSB = $("[name='ols_info']").find("[name='ontoterm']");
        varSB.append('<option value=""></option>');

        for (let varName in terms.terms) {
            varSB.append('<option value="' + varName + '"><a target="_blank" href="' + terms.terms[varName].iri + '">' + terms.terms[varName].id + '</a> - ' + terms.terms[varName].label + '</option>');
        }
    }
...
<div id="col_define_popup" hidden>
    ...
    <div class="col-sm-12">
        ...
        <div class="form-group col-sm-4">
            ...
            <div class="input-group col-sm-12">
                <select name="var_type" class="form-control" data-placeholder="Choose a variable type...">
                    ...
                    <option value="ols">Ontological term</option>
        ...
        <!-- Ontology Variable Info -->
        <div name="ols_info" hidden>
            <div class="form-group col-sm-12">
                <label class="control-label">Ontological Entity</label>
                <div class="input-group col-sm-12">
                    <select name="ontoterm" class="form-control col-def-input-item" data-placeholder="Choose a term...">
                        <option></option>
                        <option></option>
                    </select>
                </div>
