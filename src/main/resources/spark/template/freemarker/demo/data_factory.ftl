...
//            let workbook;
            ...
            let terms = {
                "terms" : {
                    <#list ontoTerms as var>
                    "${var.id}" : {
                        id : "${var.id?js_string}",
                        label: '${var.label?js_string}',
                        iri: "${var.iri?js_string}"
                    }<#sep>,</#sep>
                    </#list>
                },
                "getLabel" : function(termId) {
                    console.log("searching label for term " + termId);
                    if (this.terms[termId]) {
                        return this.terms[termId].label;
                    }
                },
                "getIri" : function (termId) {
                    console.log("searching iri for term " + termId);
                    if (this.terms[termId]){
                        return this.terms[termId].iri;
                    }
                }
            };
                        ...
                        if (roa.length && roa.length > 0) {
                            ...
                            if (tableDef.mappings.length === 0 || isChanged) {
                                for (let i = 0; i < headers.length; i++) {
                                    ...
                                    console.log("header def: " + headerDef);
                                    if (!headerDef) {
                                        headerDef = {
                                            column_header : "",
                                            column_index : i + 1,
                                    ...
                                    if (headers[i]) {
                                        ...
                                        console.log("column header: " + headerDef.column_header);
                                    }
                                        ...
                                        for (let i in selection) {
                                            for (let j = selection[i].start.col; j <= selection[i].end.col; j++) {
                                                if (mappings[j].unit_error) {
                                                    ...
                                                    console.log("icasa unit: " + icasaUnit);
                                                    if (icasaUnit) {
...
//                let colIdx = " <span class='badge'>" + (col + 1) + "</span>";
                ...
                } else if (mapping.ontoterm) {
                    console.log("found ols mapping");
                    delete mapping.icasa;
                    delete mapping.unit;
                    delete mapping.description;
                    console.log("term: " + mapping.ontoterm);
                    text = refMark + "[" + colIdx + "] " + mapping.ontoterm;
                    let termLabel = terms.getLabel(mapping.ontoterm);
                    let termIri = terms.getIri(mapping.ontoterm);
                    mapping.label = termLabel;
                    mapping.iri = termIri;
                    tooltip = "<" + mapping.ontoterm + "> " + termLabel + " (" + termIri + ")";
                } else if (!mapping || (!mapping.column_header && !mapping.icasa)) {
                    text = refMark + colIdx;
//                } else if (!mapping.icasa) {
//                    text = refMark + mapping.column_header + "[" + colIdx + "]";
                } else if (mapping.icasa) {
                    delete mapping.ontoterm;
                    delete mapping.label;
                    delete mapping.iri;
                    let varDef = icasaVarMap.getDefinition(mapping.icasa);
                    console.log("found icasa mapping: " + mapping.icasa);
                    text = refMark + "[" + colIdx + "] ";
...
//                    header.attr("class", "label label-default");
            ...
            function getColStatusClass(col, mappings) {
                if (!mappings) {
                    console.log("no mappings - retrieving mappings");
                ...
                if (mappings[col]) {
                    ...
                    } else if (mappings[col].ontoterm) {
                         console.log("found ontoterm mappings: " + mappings[col].ontoterm);
                         return "label label-onto";
                     } else if (mappings[col].icasa) {
                         console.log("found column mappings: " + mappings[col].icasa);
...
//                    centerVertical: true,
                ...
                if (agmipData.length > headerRow + 1) {
                    ...
                    for (let i = tableDef.mappings.length - 1; i > -1; i--) {
                        ...
                        if (mapping.ignored_flg) {
                            ...
                            agmipData[headerRow][mapping.column_index] = mapping.ontoterm;
                        } else if (mapping.icasa) {
                            agmipData[headerRow][mapping.column_index] = mapping.icasa;
            ...
            function openTemplateFile() {
                if (Object.keys(workbooks).length === 0) {
                    ...
                    $('<input type="file" accept=".sc2.json,.json,.sc2,.vmpr" onchange="readSC2Json(this);" multiple>').click();
                }
            }
...
//                fileMeta.file_name_local = fileName;
            ...
            function replaceOrgCode(rawData, tableDef, modifyOrgDataFlg) {
                if (!modifyOrgDataFlg) {
                    ...
                    console.log("not modified org data flag");
                }
                let mappings = tableDef.mappings;
                ...
                if (!curFileName) {
                    ...
                    bootbox.prompt({ 
                        ...
                        callback: function (result) {
                            ...
                            let text = toSC2Json();
                            if (!result.toLowerCase().endsWith(".vmpr")) {
                                sc2FileName = result + ".vmpr";
                            } else {
                                sc2FileName = result;
                            }
...
<!--                <button type="button" class="btn btn-primary" onclick="openFile()"><span class="glyphicon glyphicon-open"></span> Load</button>
    ...
    <!--                        <span class="label label-info"><strong>&nbsp;Header Row&nbsp;</strong></span>
                        ...
                        <span class="label label-onto">Ontology Term Mapped</span>
